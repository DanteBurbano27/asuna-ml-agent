...
def run_demo() -> None:
    ...
    # 4. Train Models
    print("\n[STEP 4] Fitting baseline (Logistic Regression) vs contender (Random Forest)...")
    runs = workflow.train_models(test_size=0.25, random_state=42)
    for name, ev in runs.items():
        print(f"  -> Model '{name}': ROC-AUC={ev.roc_auc:.4f}, F1={ev.f1:.4f}, Precision={ev.precision:.4f}, Recall={ev.recall:.4f}")

    # 5. Compare Models
    print("\n[STEP 5] Comparing runs and declaring champion...")
